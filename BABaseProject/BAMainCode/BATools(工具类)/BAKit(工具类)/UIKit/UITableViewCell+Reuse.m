...
+ (void )registCellWithTableView:(UITableView *)tableView
{
    ...
    UINib *nib = [UINib nibWithNibName:iden bundle:nil];
    if (nib)
    {
    ...
    {
        [tableView registerClass:[self class] forCellReuseIdentifier:iden];
    }
}

+ (__kindof UITableViewCell *)cellWithTableView:(UITableView *)tableView indexPath:(NSIndexPath *)indexPath
{
    return [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([self class]) forIndexPath:indexPath];
}

+ (__kindof UITableViewCell *)dequeueCellWithTableView:(UITableView *)tableView
{
