...
// 失败存储值
@property (nonatomic, strong) UpdatingViewStoreValue  *failedLabelStoreValue;
// 下雪
@property (nonatomic, strong) BASnowView                *snow;
// 下雪存储值
@property (nonatomic, strong) UpdatingViewStoreValue  *snowStoreValue;

@end

@implementation BAUpdatingView

- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:CGRectMake(0, 0, BA_SCREEN_WIDTH / 2.0f, BA_SCREEN_WIDTH / 2.0f + UPDATING_HEIGHY)])
    {
        [self initLabel];
        [self initSnow];
        // 关闭用户交互
...
- (void)initLabel {
    ...
    self.label = [[UILabel alloc] initWithFrame:CGRectMake(0, BA_SCREEN_WIDTH / 2.f, BA_SCREEN_WIDTH / 2.f, UPDATING_HEIGHY)];
    ...
    self.failedLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, BA_SCREEN_WIDTH / 2.f, BA_SCREEN_WIDTH / 2.f, UPDATING_HEIGHY)];
...
- (void)initSnow {
    ...
    self.snow = [[BASnowView alloc] initWithFrame:CGRectMake(0, -20, BA_SCREEN_WIDTH / 2.f, BA_SCREEN_WIDTH / 2.f)];
    ...
    self.snow.layer.mask = [CALayer createMaskLayerWithSize:CGSizeMake(BA_SCREEN_WIDTH / 2.f, BA_SCREEN_WIDTH / 2.f)
...
- (void)show
{
    [UIView animateWithDuration:1.0f animations:^{
        ...
        self.snow.alpha = 1.0f;
        self.snow.transform = CGAffineTransformMake(1.0f, 0, 0, 1.0f, 0, 0);
